//! Application, dependency, and host-resource health checks.
...
pub async fn get_health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Ok(Ok(cpu_details)) = cpu_result {
        ...
        details["cpu_usage"] =
            json!({ "status": "error", "message": "Failed to retrieve CPU usage" });
        status = "degraded";
    }
    ...
    if let Ok(Ok(mem_details)) = mem_result {
        ...
        details["memory"] =
            json!({ "status": "error", "message": "Failed to retrieve memory information" });
        status = "degraded";
    }
    ...
    if let Ok(Ok(disk_details)) = disk_result {
        ...
        details["disk_usage"] =
            json!({ "status": "error", "message": "Failed to retrieve disk usage" });
        status = "degraded";
    }
    ...
    if let Ok(Ok(process_details)) = process_result {
        ...
        details["important_processes"] =
            json!({ "status": "error", "message": "Failed to retrieve process information" });
        status = "degraded";
    }
    ...
    if let Ok(db_status) = db_result {
        ...
        details["database"] =
            json!({ "status": "error", "message": "Failed to retrieve database status" });
        status = "degraded";
    }
    ...
    if let Ok(storage_status) = storage_result {
        ...
        details["storage"] =
            json!({ "status": "error", "message": "Failed to retrieve storage status" });
        status = "degraded";
    }
    ...
    if let Ok(cache_status) = cache_result {
        ...
        details["cache"] =
            json!({ "status": "error", "message": "Failed to retrieve cache status" });
        status = "degraded";
    }
    ...
    if let Ok(Ok(net_status)) = net_result {
        ...
        details["network"] =
            json!({ "status": "error", "message": "Failed to retrieve network status" });
        status = "degraded";
    }
...
fn check_processes(system: &mut System, processes: &[&str]) -> Result<Vec<serde_json::Value>, ()> {
    ...
    let process_statuses: Vec<_> = processes
        .iter()
        .map(|&name| {
            ...
            let is_running = system
                .processes()
                .iter()
                .any(|(_, proc)| proc.name() == adjusted_name);
...
/// Verifies that a connection can be borrowed from the Redis pool and pinged.
