//! API-key rotation handlers that return a replacement credential once.
...
use crate::database::apikeys::{
    disable_apikey_in_db, fetch_existing_apikey, insert_api_key_into_db,
};
...
pub async fn rotate_apikey(
    ...
    let existing_key = fetch_existing_apikey(&state.database, user.id, uuid)
        .await
        .map_err(|e| {
