//! API-key creation handlers that persist only credential hashes.
...
pub async fn post_apikey(
    ...
    let existing_keys_count = match check_existing_api_key_count(&state.database, user.id).await {
        ...
        Err(err) => {
            error!(
                "Failed to check the amount of API keys for user {}: {}",
                user.id, err
            );
            return Err((
    ...
    if existing_keys_count >= 5 {
        ...
        return Err((
            StatusCode::BAD_REQUEST,
            Json(
                json!({ "error": "You already have 5 API keys. Please delete an existing key before creating a new one." }),
            ),
        ));
    }

    let current_date = Utc::now().naive_utc();
    let description = api_key_request
        .description
        .unwrap_or_else(|| format!("API key created on {}", current_date.format("%Y-%m-%d")));

    let expiration_date = api_key_request
        .expiration_date
