//! Fluent construction of public, optional-auth, and role-protected Axum routes.
...
impl AuthenticatedRouteBuilder {
    ...
    /// Add a GET route with optional authentication.
    /// If a valid token is present, the user is extracted; otherwise the request
    /// proceeds without a user in extensions.
    #[allow(dead_code)]
    pub fn maybe_authenticated_get<H, T>(mut self, path: &str, handler: H) -> Self
    where
        H: axum::handler::Handler<T, Arc<AppState>> + Clone + Send + Sync + 'static,
        T: 'static,
    {
    ...
    /// Add a POST route with optional authentication: the handler runs whether
    /// or not a valid session is present (it receives `Option<Extension<User>>`).
    pub fn maybe_authenticated_post<H, T>(mut self, path: &str, handler: H) -> Self
    where
        H: axum::handler::Handler<T, Arc<AppState>> + Clone + Send + Sync + 'static,
        T: 'static,
    {
