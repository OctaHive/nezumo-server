//! Redis URL parsing and deadpool connection-pool construction.
...
/// Creates the Redis connection pool from the configured URL and pool settings.
...
/// Build the validated Redis URL used both by the command pool and by the
/// dedicated async Pub/Sub connection (which cannot borrow a pooled command
/// connection while subscribed).
pub fn redis_url() -> Result<String, RedisStorageError> {
