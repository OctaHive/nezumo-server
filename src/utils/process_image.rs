//! CPU-bound image crop, resize, and WebP encoding helpers.
...
pub async fn process_image(
    ...
    let result: Result<Bytes, String> = task::spawn_blocking(move || {
        ...
        // Resize
        let stage_timer = debug.then(Instant::now);
        let resized = cropped.resize_to_fill(width, height, imageops::FilterType::Lanczos3);
        if debug {
...
fn log_time(stage: &str, timer: Instant) {
    println!(
        "{} took {:.2}ms",
        stage,
        timer.elapsed().as_micros() as f32 / 1000.0
    );
}
