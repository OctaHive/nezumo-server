//! Reusable request-field validators used by API models.
...
pub fn validate_birthday(birthday: &NaiveDate) -> Result<(), ValidationError> {
    ...
    if *birthday < earliest || *birthday > today {
        ...
        err.message =
            Some("Birthday must be within the last 120 years and not in the future.".into());
        return Err(err);
    }
    Ok(())
}
...
pub fn validate_country_code(code: &str) -> Result<(), ValidationError> {
    if !countries().contains_key(code) {
        ...
        err.message =
            Some(format!("'{}' is not a valid ISO 3166-1 alpha-2 country code.", code).into());
        return Err(err);
    }
    Ok(())
}
...
pub fn validate_language_code(code: &str) -> Result<(), ValidationError> {
    if !languages().contains_key(code) {
        ...
        err.message = Some(
            format!(
                "'{}' is not a valid language-region code (e.g., en_US, nl-NL).",
                code
            )
            .into(),
        );
