# **Setup Instructions for Nezumo on Ubuntu 24.04.01 LTS**

This guide walks you through setting up the **Nezumo** project on an Ubuntu system. It includes the installation of necessary tools like Rust, SQLx, PostgreSQL, Redis, MinIO, and Mail (SMTP). 
...
### **6a. Install MuPDF (`mutool`) for PDF import**
The `POST /boards/{id}/pdf` endpoint converts each PDF page to an SVG using the
`mutool` CLI. Without it the endpoint returns `"PDF conversion tool unavailable"`.
```sh
sudo apt install -y mupdf-tools
mutool -v   # verify
```
If `mutool` is not on the backend service's PATH, set its full path in the
backend `.env`: `MUTOOL_BIN=/usr/bin/mutool`. Restart the backend after install.
...
### **10. Clone the Nezumo repository**
Clone the Nezumo repository from GitHub:
```sh
git clone https://github.com/OctaHive/nezumo-server.git
```
...
### **13. Build and run the Nezumo application**
Build and run the Nezumo application:
```sh
cargo run
```
...
Redis is used as a caching mechanism in Nezumo. To set it up:
...
Nezumo can send emails for actions like user registration, password reset, etc. To configure the SMTP settings:
...
#### **Run the Nezumo Application**
For running the application in development, follow these steps.

1. Run Nezumo by executing the following command in the project folder:
   ```sh
   cargo run
   ```
2. If everything starts up without errors, test your Nezumo application by visiting: 
   - `http://127.0.0.1:8000`
  
With these instructions, you should now have **Nezumo**, **Redis**, **MinIO**, and **Mail (SMTP)** properly set up on your **Ubuntu 24.04.01 LTS** server.

#### **Release the Nezumo Application**
To release your Nezumo application (i.e., compile it for production or deploy it), follow these steps:
...
2. **Check the Build Output**
   - After the build completes, navigate to the `target/release` directory:
     ...
     cd target/release
     ```
   - You should see an executable file named after your project (e.g., `nezumo.exe`).
...
### **18. Reverse proxy (nginx) — upload size limit**
nginx defaults `client_max_body_size` to **1 MB**, which rejects file/PDF
uploads with **`413 Request Entity Too Large`** before the request reaches the
backend. Raise it in the API location (or the whole `server {}` block):
```nginx
location /api/ {
    proxy_pass http://127.0.0.1:3000;
    client_max_body_size 50M;        # allow image / PDF uploads
    proxy_request_buffering off;     # optional: stream large bodies
    # ... existing proxy_set_header / timeout directives ...
}
```
Then reload nginx:
```sh
...
```
The backend already allows up to 50 MB on these routes, so nginx is the binding
limit.
